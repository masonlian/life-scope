...
class match_result:
    ...
    return_steady_matrix:list[steady_probility]
    return_most_matched_city:city_score
    return_user_major_activity :list[steady_probility]
    return_data_for_pie :list[data_for_pie]
  
@dataclass
class similarity_and_pie_data:
        cosine_similarity:float
        data_for_pie_list:list[data_for_pie]
...
#    這邊進來是一個dict而非object
   ...
   steady_matrix = compute_transition_matrix(data1) #回傳一個機率矩陣
   
   #在python中傳遞list是以參考方式傳遞，因此在這邊要特別注意不要直接操作到原本的steady_matrix
   #steady_matrix_to_return=steady_matrix 像這樣的語句就只是建立一個新的參考指向同一個List物件而已
   
   steady_matrix_for_return = copy.deepcopy(steady_matrix)
   
   print("穩定機率總和檢查:",sum([m.probility for m in steady_matrix]), end="\n")
 
 
   user_major_activity = major_activity(steady_matrix)
   user_major_activity_for_return = copy.deepcopy(user_major_activity)
   ...
   for city in city_to_match:
     ...
     matrix_similarity_and_pie_data = compute_steady_matrix_similarity(user_major_activity,city.steady_vector) #回傳一個穩定狀態餘弦相似度
     
     match_score_result= (0.5*matrix_similarity_and_pie_data.cosine_similarity) + (0.3*distance_similarity) + (0.2*entroypy_similarity)
   ...
   match_return = match_result(return_entropy=entropy, return_avg_distance=avg_distance,return_steady_matrix=steady_matrix_for_return,return_most_matched_city=most_matched_city,return_user_major_activity = user_major_activity_for_return,return_data_for_pie= matrix_similarity_and_pie_data.data_for_pie_list)
...
def compute_steady_matrix_similarity(user_major_activity, city_steady_vector):
    ...
    for spot in user_major_activity:
        ...
        if spot.public_name  == "向陽路182號":
            spot.poi = "Work"
            prefer_spot = data_for_pie(spot=spot.poi, probility=spot.probility)
            data_for_pie_list.append(prefer_spot)
            
            
        if spot.public_name == "光啟路247巷19號":
            spot.poi = "Home"
            prefer_spot = data_for_pie(spot=spot.poi, probility=spot.probility)
            data_for_pie_list.append(prefer_spot)
    ...
    if (len(city_aligned) != len (user_major_activity)):
      ...
      data_for_return = similarity_and_pie_data(cosine_similarity=cosine_simularity, data_for_pie_list=data_for_pie_list)
