...
public class CollectDataController {
    ...
    public ResponseEntity<List<FinalLocationJournal>> getTableData(@RequestParam String month , @RequestParam String day ) throws  Exception{

       List<FinalLocationJournal> locationJournalList  = locationService.getJournalByDate(month,day);
       if(locationJournalList.size()>0){
           return ResponseEntity.status(HttpStatus.OK).body(locationJournalList);
       }
