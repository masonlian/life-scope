...
public class LocationDaoImpl implements LocationDao {
   ...
   public Integer enrollLocation(LocationData locationData){
    ...
    @Override
    public List<FinalLocationJournal> getJournalByDate(String month, String day){

       System.out.println("傳入的月份為:"+month+"傳入的日期是:"+day);
       if(month != null  & day!= null){
        String sql = "SELECT *   FROM  location_journal   WHERE MONTH(acquiring_time) = :month AND DAY(acquiring_time) = :day ";
        Map<String,Object> map = new HashMap<>();
        map.put("month", month);
        map.put("day", day);
        List<FinalLocationJournal> finalLocationJournalList =  namedParameterJdbcTemplate.query(sql,map,new FinalLocationJournalRowMapper());
        if(finalLocationJournalList.size()>0){
            return finalLocationJournalList;
        }
        else log.warn("資料撈取失敗");
       }
       else log.warn("年、月資料有問題。");
               return null;
